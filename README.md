# OXLAB & Taskion

## OXLAB
OXLAB is a team of enthusiasts focused on modern web technologies. Our mission is to create useful, secure, and user-friendly tools for everyone.

---

## What is Taskion?

**Taskion** is a modern, fast, and intuitive project management tool for teams of all sizes.  
It is fully self-hosted, giving you complete control over your data and deployment.

### Key Features
- Kanban board with drag & drop
- Gantt chart for project timelines
- Task management with priorities, deadlines, and attachments
- Comments and notifications for team collaboration
- User roles and permissions
- Multi-language support (Czech, English, German, Spanish)
- Dark & light mode
- Personal notes and reminders
- Dashboard overview
- Secure authentication (JWT, hashed passwords)
- Free updates

---

## How to Get Taskion

### 1. For Personal / Non-Commercial Use
- Download the prebuilt installation package
- Easy setup, ready to use on your own server
- Free for personal and non-commercial projects

[Download Prebuilt Installation](https://oxlab.app/pricing.php)

---

### 2. For Commercial Use
- Get full access to the source code
- Modify, extend, and integrate Taskion into your company or client projects
- Free for commercial use, but voluntary support is welcome

[Get Source Code](https://oxlab.app/pricing.php)

---

## Documentation & Support

- [FAQ](https://oxlab.app/faq.php) — Frequently Asked Questions
- [Contact](https://oxlab.app/contact.php) — Ask us anything, we usually reply within 1–2 business days

---

## License

Taskion is free for both personal and commercial use.  
**You are not allowed to resell Taskion or offer it as your own product.**  
See the [License Agreement](https://oxlab.app/licence.php) for details.

---

## Stay Connected

- GitHub: [github.com/oxlab](https://github.com/oxlab)
- Email: office@oxlab.app
